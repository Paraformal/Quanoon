...
def get_view_link(url):
    try:
        ...
        if response.status_code == 200:
            ...
            a_tag = soup.find('a', {'id': 'MainContent_aView'})
            if a_tag and a_tag.has_attr('href'):
                return f"http://77.42.251.205/{a_tag['href'].lstrip('/')}"
        return None
...
def process_year_wrapper(year, batch_size=10, max_workers=5):
    ...
    filtered_urls = [url for url in urls if url.startswith("http://77.42.251.205/ViewRulePage.aspx?ID=")]
    if not filtered_urls:
        logger.info(f"No valid Law.aspx links found for year {year}, skipping.")
        return year, 0
    return process_year(year, filtered_urls, batch_size=batch_size, max_workers=max_workers)
